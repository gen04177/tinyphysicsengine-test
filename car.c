...
void printHelp(void)
{
    printf("Car: example program for tinyphysicsengine.\n\n");
...
int main(void)
{
  ...
  while (helper_running)
  {
    ...
    // Trigger buttons for acceleration and braking
    ...
    else if (SDL_GameControllerGetAxis(controller, SDL_CONTROLLER_AXIS_TRIGGERLEFT) > 8000)
    {
      carBody->joints[0].velocity[0] -= (carForw.x * ACCELERATION) / TPE_F;
      carBody->joints[0].velocity[1] -= (carForw.y * ACCELERATION) / TPE_F;
      carBody->joints[0].velocity[2] -= (carForw.z * ACCELERATION) / TPE_F;
